//HERE BE VARIABLES
var xOff=450,yOff=0,zoom=2.6,iterations=50,mode=0,prevH=0;
...
function generate(){
  ...
  for (var h=scan;h<scan+10;h++){
    for (var w=0;w<width;w++){
      a2=(w-width/2-xOff/zoom)*(zoom/width);
      b2=(h-height/2-yOff/zoom)*(zoom/height);
      if (isMandel){
        a=a2;
        b=b2;
      }else{
...
function paint2(event){
  ...
  if (xRes>0&&yRes>0&&mode==0||move){
    ...
    a2=(tX-width/2-xOff/zoom)*(zoom/width);
    b2=(tY-height/2-yOff/zoom)*(zoom/height);
    if (isMandel){
      a=a2;
      b=b2;
    }else{
